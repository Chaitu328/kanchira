...
// import { getAddress, getCouponByCode, createPayment,
// getCart,
//   updateCartQuantity,
//   removeItemCart,   // regular admin coupons  → GET /getCouponByCode/:code
//   validateSuperCoupon
// } from '../services/api'
...
//   }}
// >
//   <div className="text-sm font-bold text-[#800000] mb-3">
//     🎟️ Apply Coupon
//   </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-10 text-center max-w-sm w-full">
        {status === "success" ? (
          ...
          <>
            ...
            <p className="text-gray-500 text-sm mb-6">
              We couldn&apos;t read payment status from the callback.
            </p>
          </>
        ) : (
          <>
        ...
        <button className="btn-maroon" onClick={() => navigate("/")}>
          Continue Shopping
        </button>
        <button
          className="mt-3 text-sm text-[#800000] hover:underline block mx-auto"
...
function storeAddress(addr) {
  try {
    localStorage.setItem("kanchira_address", JSON.stringify(addr));
  } catch {}
}
...
export function CheckoutPage() {
  ...
  const { user, cartItems, setLoginModalOpen, setCartItems, loadCart } =
    useApp();
  ...
  const [address, setAddress] = useState(
    () => location?.state?.address || readStoredAddress(),
  );
  const [showAddressModal, setShowAddressModal] = useState(false);
  ...
  useEffect(() => {
    ...
    if (location?.state?.address) {
      storeAddress(location.state.address);
      return;
    }
    if (address) return;
    getAddress()
  ...
  const buyNowData = useMemo(() => {
    if (mode !== "buynow") return null;
    try {
      return JSON.parse(localStorage.getItem("buynow_product") || "null");
    } catch {
      return null;
    }
  }, [mode]);
  ...
  const lines = useMemo(() => {
    if (mode === "buynow") {
      ...
      const image =
        ...
        "";
      const price =
        Number(selectedSize?.finalPrice ?? selectedSize?.price ?? 0) || 0;
      return [
        {
          index: 0,
          name: product?.name || "Product",
    ...
    return items.map((item, index) => {
      const product =
        item?.productId || item?.product || item?.productData || {};
      return {
        index,
        name: product?.name || item?.name || "Product",
        image: item?.image || product?.image || product?.thumbnail || "",
  ...
  const spinDiscountAmt =
    spinDiscountApplied && spinDiscount
      ? (subtotal * spinDiscount.value) / 100
      : 0;

  // Only one discount active — spin takes priority if both somehow set
  const activeDiscountAmt = spinDiscountApplied
    ? spinDiscountAmt
    : couponDiscountAmt;
  const deliveryCharge = 0;
  const grandTotal = Math.round(
    Math.max(0, subtotal - activeDiscountAmt) + deliveryCharge,
  );
  ...
  const buildOrderPayload = (paymentMethod) => ({
    ...
    couponCode: couponApplied && !spinDiscountApplied ? couponCode : "",
    couponDiscount:
      couponApplied && !spinDiscountApplied ? Math.round(couponDiscountAmt) : 0,
    spinDiscount: spinDiscountApplied ? Math.round(spinDiscountAmt) : 0,
    festivalDiscount: 0,
  });
  ...
  const applyCouponCode = async (rawCode) => {
    ...
    try {
      // ── 1. Try SuperAdmin coupon first ──────────────────────────────────
      try {
        const r = await validateSuperCoupon(code, subtotal); // ✅ pass string + amount
        const d = r.data?.data || r.data;
        const discountType = d?.discountType || "flat";
        const discountValue = Number(
          d?.discountValue ?? d?.discountAmount ?? 0,
        );
        ...
        // Only fall through on 404/400 — re-throw anything else (e.g. 500 network error)
        if (status !== 404 && status !== 400) throw superErr;
        const msg = superErr?.response?.data?.message || "";
        if (
          msg &&
        ...
        // fall through to admin coupon
      }

      // ── 2. Fallback: regular admin coupon ───────────────────────────────
      const r = await getCouponByCode(code);
      ...
      const discountValue = Number(c.value ?? c.discountValue ?? 0);
  ...
  const handlePaymentSelect = (method) => {
    setSelectedPaymentMethod(method);
    // Do NOT auto-place — user must confirm via the Place Order button
  };
  ...
  const finalizeCODOrder = async () => {
    ...
    try {
      ...
      // ── Clear cart in backend (DB) after successful order ─────────────
      if (mode !== "buynow") {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-xl border p-6 text-center max-w-sm w-full">
          <p className="text-gray-600 text-sm">Please login to continue.</p>
          <button className="btn-maroon mt-4" onClick={() => navigate("/cart")}>
            Go to Cart
          </button>
        </div>
      </div>
  ...
  if (mode === "buynow" && !buyNowData?.product) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-xl border p-6 text-center max-w-sm w-full">
          <p className="text-gray-600 text-sm">No product selected.</p>
          <button className="btn-maroon mt-4" onClick={() => navigate("/")}>
            Continue Shopping
          </button>
        </div>
      </div>
  ...
  const paymentMethods = [
    ...
    },
    {
      ...
      icon: "💵",
      description: `Pay ₹${grandTotal} on delivery`,
    },
  ];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="checkout-topbar">
          ...
          {address && (
            <span className="text-sm text-gray-600">
              {[
                address.currentAddress || address.street,
                address.city,
                address.state,
                address.pincode,
              ]
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pb-10">
          ...
          <div className="lg:col-span-7 space-y-4">
                  ...
                  <div className="space-y-4">
                    {lines.map((line) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold capitalize line-clamp-2">
                            {line.name}
                          </p>
                          <div className="text-xs text-gray-500 mt-1 flex gap-3 flex-wrap">
                            {line.size && <span>Size: {line.size}</span>}
                          ...
                          <div className="mt-3 flex items-center justify-between gap-3">
                            ...
                            <span className="font-bold">
                              ₹{Math.round(line.price * (line.qty || 1))}
                            </span>
                          </div>
                        </div>
                      </div>
            ...
            {step === "address" && (
              <div className="bg-white border border-gray-200 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="font-extrabold text-[#800000]">
                    Delivery Address
                  </h2>
                  <button
                    className="text-sm text-[#800000] hover:underline"
                ...
                {showPaymentOptions ? (
                  <div className="border-2 border-[#800000] rounded-xl p-4 mb-4 bg-[#fff5f5]">
                    ...
                    <div className="space-y-2">
                      {paymentMethods.map((method) => (
                        <div
                          key={method.id}
                          onClick={() =>
                            !isPlacing && handlePaymentSelect(method.id)
                          }
                          className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all
                            ${isPlacing ? "opacity-50 cursor-not-allowed" : ""}
                            ${
                    ...
                    {selectedPaymentMethod === "COD" && (
                      <button
                        onClick={finalizeCODOrder}
                        disabled={isPlacing}
                        className="w-full mt-4 bg-yellow-400 text-black py-3 rounded-lg font-bold hover:bg-yellow-500 transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
                      >
                        {isPlacing ? (
                          <>
                  ...
                  <div className="text-center p-6 border border-dashed border-gray-300 rounded-xl">
                    ...
                    <p className="text-xs text-gray-400">
                      Click the button below to see payment options
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="lg:col-span-5">
            <div className="checkout-order-card p-6 lg:sticky lg:top-24">
                ...
                {spinDiscountApplied && spinDiscount && (
                  <div className="checkout-row text-green-600">
                    <span className="muted">
                      Spin Discount ({spinDiscount.label})
                    </span>
                    <span>-₹{Math.round(spinDiscountAmt)}</span>
                  </div>
                )}
              ...
              <div className="mt-3 checkout-pill-warning">
