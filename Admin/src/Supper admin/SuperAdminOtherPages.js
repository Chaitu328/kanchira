...
import {
  superAdminGetBanners as getBanners,
  superAdminUpdateBanner as updateBanner,
  superAdminCreateBanner as createBanner,
  superAdminDeleteBanner as deleteBanner,
  superAdminGetAllReviews as getReviews,
  superAdminAddReview as addReview,
  superAdminUpdateReview as updateReview,
  superAdminDeleteReview as deleteReview,
  superAdminGetAllUsers as getAllUsers,
  superAdminDeleteUser as deleteUser,
  superAdminGetOrders as getOrders,
  superAdminUpdateOrderStatus as updateOrderStatus,
  superAdminGetLogo as getLogo,
  superAdminCreateLogo as createLogo,
  superAdminUpdateLogo as updateLogo,
  superAdminDeleteLogo as deleteLogo,
  superAdminGetPincodes as getPincodes,
  superAdminSearchPincodes as searchPincodes,
  superAdminCreatePincode as createPincode,
  superAdminDeletePincode as deletePincode,
  superAdminBulkUploadPincodes as bulkUploadPincodes,
  superAdminExportPincodes as exportPincodes,
  superAdminDownloadPincodeSampleTemplate as downloadPincodeSampleTemplate,
  superAdminGetCoupons as getCoupons,
  superAdminCreateCoupon as createCoupon,
  superAdminDeleteCoupon as deleteCoupon,
  getCouponByCode,
  superAdminGetAddresses as getAddresses,
...
export function CouponCodes() {
  ...
  const handleCheckCoupon = async (e) => {
    ...
    setCheckResult(null);
    try {
      const r = await getCouponByCode(checkCode.trim().toUpperCase());
      const coupon = r.data;
      const expired =
      ...
      if (expired) {
        ...
        setCheckResult({
          status: "valid",
          message: `Valid! ${coupon.type === "percentage" ? `${coupon.value}% OFF` : `₹${coupon.value} OFF`} — expires ${new Date(coupon.expiryDate).toLocaleDateString()}`,
        });
      }
    } catch (err) {
      ...
      if (msg.toLowerCase().includes("already used")) {
        ...
        setCheckResult({
          status: "expired",
          message: "This coupon code does not exist or is no longer valid.",
        });
      }
    } finally {
      setChecking(false);
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Status",
      render: (r) => {
        const expired = r.expiryDate && new Date() > new Date(r.expiryDate);
        if (expired) {
          return (
        ...
        return <Badge value={r.active} truthy="Active" falsy="Inactive" />;
      },
    },
    {
      key: "actions",
