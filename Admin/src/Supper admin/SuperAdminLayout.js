...
import { logoutSuperAdmin } from "../services/superAdminApi";
...
const SA_NAV = [
  ...
  { label: 'Special Coupons',  icon: 'fa-star',         path: '/superadmin/coupons' },
   { label: 'Used Coupons',    icon: 'fa-history',       path: '/superadmin/used-coupons' },
  { label: ' Special Coupon Used ',    icon: 'fa-table',    path: '/superadmin/coupon-usage-table' },
...
export default function SuperAdminLayout() {
  ...
  const handleLogout = async () => {
    try { await logoutSuperAdmin(); } catch {}
    logout();
      ...
      >
        ...
        <div className="flex flex-col items-center pt-4 pb-3 border-b" style={{ borderColor: 'rgba(210,174,78,0.4)' }}>
          <div className="flex items-center justify-center w-full px-3">
            <div className="w-14 h-14 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden border-2" style={{ borderColor: '#D2AE4E', background: 'linear-gradient(135deg, #D2AE4E, #b8941d)' }}>
        ...
        <header className="sticky top-0 z-40 flex items-center justify-between px-6 py-3 shadow-sm" style={{ backgroundColor: '#3b0000' }}>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-white font-semibold text-sm hidden sm:block">
              {SA_NAV.find(n => location.pathname === n.path)?.label || 'Super Admin Panel'}
            </h1>
          </div>
          <div className="flex items-center gap-3">
