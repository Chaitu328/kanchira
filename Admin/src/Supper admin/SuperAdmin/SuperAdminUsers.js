...
export default function SuperAdminUsers() {
  ...
  const loadAll = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [usersRes, couponRes, ordersRes, allCouponsRes] =
      ...
      setSuperCoupons(
        couponRes.status === "fulfilled"
          ? couponRes.value.data?.data?.coupons || []
          : [],
      );
      setOrders(
      ...
      setAllCoupons(
        allCouponsRes.status === "fulfilled"
          ? allCouponsRes.value.data || []
          : [],
      );
    } catch (err) {
  ...
  const getUserRevenue = (uid) =>
    getUserOrders(uid).reduce((s, o) => s + (o.totalAmount || 0), 0);
  ...
  const getUserSuperCoupon = (u) => {
    ...
    const o = getUserOrders(u._id).find(
      (ord) => ord.couponCode && superCouponCodes.has(ord.couponCode),
    );
  ...
  const initials = (u) =>
    ((u.name || u.phone || u.email || "?")[0] || "?").toUpperCase();
  const avatarColor = (u) => {
  ...
  const Skeleton = ({ rows = 5 }) => (
    <div className="space-y-3">
      {[...Array(rows)].map((_, i) => (
        <div
          key={i}
          className="h-14 rounded-xl animate-pulse bg-white shadow-sm"
        />
      ))}
    </div>
  );
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ backgroundColor: MAROON + "15" }}>
                ...
                <th className="text-right px-4 py-3 text-xs font-bold text-gray-600 uppercase hidden lg:table-cell">
                  Orders
                </th>
                <th className="text-right px-4 py-3 text-xs font-bold text-gray-600 uppercase hidden lg:table-cell">
                  Revenue
              ...
              {filtered.map((u, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className="px-4 py-3 text-right hidden lg:table-cell font-semibold"
                      style={{ color: "#16a34a" }}
                    >
              ...
              {superCoupons.map((c, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className="font-black font-mono tracking-wider px-2 py-1 rounded text-xs"
              ...
              {filteredUsedCoupons.map((c, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className="font-black font-mono px-2 py-1 rounded text-xs"
                        style={{ backgroundColor: "#fef3c7", color: "#92400e" }}
                      >
                        {c.code}
                      </span>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs px-2 py-0.5 rounded font-medium ${c.type === "percentage" ? "bg-blue-100 text-blue-700" : "bg-green-100 text-green-700"}`}
                      >
                        {c.type}
                      </span>
                    </td>
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {users.length} users · {superCoupons.length} special coupons ·{" "}
            {allCoupons.length} total coupons
          </p>
        </div>
      ...
      <div className="flex gap-1 mb-5 bg-white rounded-xl p-1 shadow-sm overflow-x-auto border border-amber-100">
        {[
          ...
          {
            key: TABS.SUPER_COUPONS,
            label: "Super Coupons",
            icon: "fa-star",
            count: superCoupons.length,
          },
          {
            key: TABS.ALL_COUPONS,
            label: "All Coupons",
            icon: "fa-ticket",
            count: allCoupons.length,
          },
          {
            key: TABS.USED_COUPONS,
            label: "Used Coupons",
            icon: "fa-check",
            count: usedCouponUsers.length,
          },
        ].map((t) => (
          <button
            key={t.key}
            ...
            style={{
              backgroundColor: activeTab === t.key ? MAROON : "transparent",
          ...
          >
            ...
            <span
              ...
              style={{
                backgroundColor:
                  activeTab === t.key ? "rgba(255,255,255,0.25)" : "#f3f4f6",
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b">
              ...
              <button
                ...
                className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center border-0 cursor-pointer"
              >
            ...
            <div className="p-5 space-y-4">
              <div className="flex items-center gap-4">
                <div
                  ...
                  style={{ backgroundColor: avatarColor(selected) }}
                >
                  {initials(selected)}
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  {
                    label: "Orders",
                    value: getUserOrders(selected._id).length,
                    color: MAROON,
                  },
                  {
                    label: "Revenue",
                    value:
                      "₹" +
                      getUserRevenue(selected._id).toLocaleString("en-IN"),
                    color: "#16a34a",
                  },
                ].map((s) => (
                  <div
