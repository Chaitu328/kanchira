...
//     { key: "super admin coupon code", label: "Coupon Code", searchable: true },
//         { key: "coupon code", label: "Coupon Code", searchable: true },
...
//   const pendingOrders   = data.filter(o => o.status === "Pending" || !o.status).length;

//   const filteredData = filter === "All" ? data : data.filter(o => (o.status || "Pending") === filter);
...
//           {[
//             { label: "Total Orders",   value: data.length,                          icon: "fa-shopping-cart", color: "#640101" },
//             { label: "Total Sales",    value: `₹${totalSales.toLocaleString("en-IN")}`, icon: "fa-inr",      color: "#975607" },
//             { label: "Completed",      value: completedOrders,                      icon: "fa-check-circle",  color: "#376D5C" },
//             { label: "Pending",        value: pendingOrders,                        icon: "fa-clock-o",       color: "#D2AE4E" },
//           ].map((s, i) => (
//             <div key={i} className="rounded-xl p-4 text-white flex items-center gap-3 shadow" style={{ backgroundColor: s.color }}>
...
//               style={filter === s ? { backgroundColor: "#640101", borderColor: "#640101" } : {}}>
//               {s}
//               {s !== "All" && (
//                 <span className="ml-1 opacity-70">({data.filter(o => (o.status || "Pending") === s).length})</span>
//               )}
//             </button>
//           ))}
//         </div>
...
//           <p className="text-gray-400 font-medium">{filter === "All" ? "No orders yet" : `No ${filter} orders`}</p>
//         </div>
//       ) : (
...
//                 const items = Array.isArray(o.items) ? o.items : Array.isArray(o.orderItems) ? o.orderItems : Array.isArray(o.products) ? o.products : [];
...
//                             <i className={`fa fa-chevron-${isOpen ? "up" : "down"} text-xs text-gray-500`} />
//                           </button>
//                         )}
//                       </td>
//                       <td className="px-3 py-3 text-sm text-gray-400">{i + 1}</td>
//                       <td className="px-3 py-3 font-mono text-xs text-gray-700">{o._id?.slice(-8).toUpperCase() || "—"}</td>
//                       <td className="px-3 py-3 text-sm text-gray-700 font-medium whitespace-nowrap">{o.address?.fullName || o.userId || "—"}</td>
//                       <td className="px-3 py-3 text-sm text-gray-500">{items.length > 0 ? <span className="bg-amber-100 text-amber-700 text-xs px-2 py-0.5 rounded-full font-semibold">{items.length} item{items.length > 1 ? "s" : ""}</span> : "—"}</td>
//                       <td className="px-3 py-3 text-sm font-semibold text-gray-800 whitespace-nowrap">{o.totalAmount ? `₹${Number(o.totalAmount).toLocaleString("en-IN")}` : "—"}</td>
//                       <td className="px-3 py-3 text-sm text-gray-500 whitespace-nowrap">{o.paymentMethod || "—"}</td>
//                       <td className="px-3 py-3">
//                         <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${STATUS_COLORS[o.status] || "bg-gray-100 text-gray-600"}`}>
//                           {o.status || "Pending"}
//                         </span>
//                       </td>
//                       <td className="px-3 py-3 text-xs text-gray-400 whitespace-nowrap">{o.createdAt ? new Date(o.createdAt).toLocaleDateString("en-IN") : "—"}</td>
//                       <td className="px-3 py-3">
//                         <select className="kanchira-input w-auto text-xs py-1"
//                           value={o.status || "Pending"}
//                           onChange={(e) => handleStatusChange(o._id, e.target.value)}>
//                           {["Pending","Confirmed","Shipped","Completed","Cancelled"].map(s => <option key={s} value={s}>{s}</option>)}
//                         </select>
//                       </td>
//                     </tr>
//                     {/* Expanded items row */}
...
//                       <tr>
//                         <td colSpan={10} className="px-6 pb-4 pt-0 bg-amber-50/60">
...
//                               <i className="fa fa-list-ul" /> Order Items — {o.address?.fullName || "Customer"}
//                             </div>
//                             <table className="w-full text-sm">
//                               <thead>
//                                 <tr className="bg-white border-b border-amber-100">
//                                   <th className="px-4 py-2 text-left text-xs text-gray-500 font-semibold">#</th>
//                                   <th className="px-4 py-2 text-left text-xs text-gray-500 font-semibold">Product Name</th>
//                                   <th className="px-4 py-2 text-left text-xs text-gray-500 font-semibold">Image</th>
//                                   <th className="px-4 py-2 text-left text-xs text-gray-500 font-semibold">Qty</th>
//                                   <th className="px-4 py-2 text-left text-xs text-gray-500 font-semibold">Price</th>
//                                   <th className="px-4 py-2 text-left text-xs text-gray-500 font-semibold">Subtotal</th>
//                                 </tr>
//                               </thead>
//                               <tbody>
//                                 {items.map((item, j) => {
//                                   const name = item.productName || item.name || item.title || item.productId?.name || "Product";
//                                   const img = item.image || item.img || item.productImage || item.productId?.images?.[0] || null;
//                                   const qty = item.quantity || item.qty || 1;
//                                   const price = item.price || item.productPrice || item.productId?.price || 0;
//                                   const subtotal = qty * Number(price);
//                                   return (
//                                     <tr key={j} className="border-b border-amber-50 last:border-0 hover:bg-white">
//                                       <td className="px-4 py-2 text-gray-400 text-xs">{j + 1}</td>
//                                       <td className="px-4 py-2 font-medium text-gray-800">{name}</td>
//                                       <td className="px-4 py-2">
//                                         {img ? <img src={img} alt="" className="w-10 h-10 object-cover rounded" /> : <span className="text-gray-300 text-xs">—</span>}
//                                       </td>
//                                       <td className="px-4 py-2 text-gray-700">{qty}</td>
//                                       <td className="px-4 py-2 text-gray-700">{price ? `₹${Number(price).toLocaleString("en-IN")}` : "—"}</td>
//                                       <td className="px-4 py-2 font-semibold text-gray-800">{subtotal ? `₹${subtotal.toLocaleString("en-IN")}` : "—"}</td>
//                                     </tr>
//                                   );
//                                 })}
//                               </tbody>
...
//         productName: "—", image: null, qty: "—", price: "—",
...
//       const img  = item.image || item.img || item.productImage || item.productId?.images?.[0] || null;
//       const qty  = item.quantity || item.qty || 1;
//       const price = Number(item.price || item.productPrice || item.productId?.price || 0);
//       return {
//         orderId, customer, date, status,
//         productName: name, image: img,
...
//           {[
//             { label: "Total Sales",  value: `₹${totalSales.toLocaleString("en-IN")}`, icon: "fa-inr",           color: "#640101" },
//             { label: "Total Orders", value: data.length,                               icon: "fa-shopping-cart", color: "#975607" },
//             { label: "Total Items Sold", value: totalItems,                            icon: "fa-cube",          color: "#376D5C" },
//           ].map((s, i) => (
//             <div key={i} className="rounded-xl p-4 text-white flex items-center gap-3 shadow" style={{ backgroundColor: s.color }}>
//               <div className="w-10 h-10 rounded-lg bg-white/20 flex items-center justify-center flex-shrink-0">
//                 <i className={`fa ${s.icon} text-yellow-200`} />
...
//             <thead>
//               <tr>
//                 {["#", "Order ID", "Customer", "Status", "Product Name", "Image", "Qty", "Price", "Subtotal", "Date"].map(h => (
//                   <th key={h} className="px-3 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap">{h}</th>
//                 ))}
//               </tr>
//             </thead>
//             <tbody>
//               {filtered.map((row, i) => (
//                 <tr key={i} className={`hover:bg-amber-50/40 transition ${row.isFirstItem ? "border-t-2 border-amber-100" : ""}`}>
//                   <td className="px-3 py-2 text-xs text-gray-400">{i + 1}</td>
//                   <td className="px-3 py-2 font-mono text-xs text-gray-600">{row.orderId}</td>
//                   <td className="px-3 py-2 text-sm font-semibold text-gray-800 whitespace-nowrap">{row.customer}</td>
//                   <td className="px-3 py-2">
//                     <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${STATUS_COLORS[row.status] || "bg-gray-100 text-gray-600"}`}>
//                       {row.status}
//                     </span>
//                   </td>
//                   <td className="px-3 py-2 text-sm text-gray-700 font-medium">{row.productName}</td>
//                   <td className="px-3 py-2">
//                     {row.image ? <img src={row.image} alt="" className="w-10 h-10 object-cover rounded" /> : <span className="text-gray-300 text-xs">—</span>}
//                   </td>
//                   <td className="px-3 py-2 text-sm text-gray-600">{row.qty}</td>
//                   <td className="px-3 py-2 text-sm text-gray-600 whitespace-nowrap">{row.price !== "—" ? `₹${Number(row.price).toLocaleString("en-IN")}` : "—"}</td>
//                   <td className="px-3 py-2 text-sm font-bold text-gray-800 whitespace-nowrap">{row.subtotal ? `₹${row.subtotal.toLocaleString("en-IN")}` : "—"}</td>
//                   <td className="px-3 py-2 text-xs text-gray-400 whitespace-nowrap">{row.date}</td>
//                 </tr>
//               ))}
//             </tbody>
//             {/* Grand total footer */}
...
//                 <td colSpan={8} className="px-3 py-3 text-right text-sm font-bold text-gray-700">Grand Total</td>
//                 <td className="px-3 py-3 text-sm font-black" style={{ color: "#640101" }}>
...
//               <span className="font-mono font-bold text-red-700">{deleteTarget.code}</span>?
//             </p>
//             <div className="flex gap-3 justify-end">
//               <button
//                 className="btn-secondary"
